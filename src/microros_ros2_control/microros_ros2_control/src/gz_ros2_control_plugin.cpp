...
#include "microros_ros2_control/gz_ros2_control_plugin.hpp"
...
public:
  ...
  std::shared_ptr<pluginlib::ClassLoader<microros_ros2_control::GazeboSimSystemInterface>>
    ...
    switch (jointType->Data()) {
      ...
      case sdf::JointType::FIXED: {
        RCLCPP_INFO(node_->get_logger(),
                    "[microros_ros2_control] Fixed joint [%s] (Entity=%lu)] is skipped",
      ...
      case sdf::JointType::UNIVERSAL: {
        RCLCPP_WARN(node_->get_logger(),
                    "[microros_ros2_control] Joint [%s] (Entity=%lu)] is of "
                    "unsupported type."
      ...
      default: {
        RCLCPP_WARN(node_->get_logger(),
                    "[microros_ros2_control] Joint [%s] (Entity=%lu)] is of unknown type",
                    jointName.c_str(), jointEntity);
        continue;
      }
    }
  ...
  while (urdf_string.empty()) {
    ...
    if (!urdf_string.empty()) {
      ...
      RCLCPP_ERROR(node_->get_logger(),
                   "microros_ros2_control plugin is waiting for model"
                   " URDF in parameter [%s] on the ROS param server.",
                   this->robot_description_.c_str());
    }
    std::this_thread::sleep_for(std::chrono::microseconds(100000));
  }
  RCLCPP_DEBUG(node_->get_logger(), "Received URDF from param server: %s",
               urdf_string.c_str());  // буквально urdf
  return urdf_string;
}
...
void GazeboSimROS2ControlPlugin::Configure(const sim::Entity& _entity,
  ...
  if (!model.Valid(_ecm)) {
    RCLCPP_ERROR(logger,
                 "[microros_ros2_control] Failed to initialize because [%s] "
                 "(Entity=%lu)] is not a model."
                 "Please make sure that microros_ros2_control is attached to a "
                 "valid model.",
  ...
  if (paramFileName.empty()) {
    RCLCPP_ERROR(logger,
                 "microros_ros2_control found an empty parameters "
  ...
  std::string node_name = "microros_ros2_control";
  ...
  // set the robot description as argument to propagate it among controller
  // manager and controllers Remove all comments via regex pattern to match XML
  // comments, including newlines
  const std::regex comment_pattern(R"(<!--[\s\S]*?-->)");
  const auto rb_arg =
      std::string("robot_description:=") + std::regex_replace(urdf_string, comment_pattern, "");
  ...
  rcl_ret_t rcl_ret = rcl_parse_arguments(static_cast<int>(argv.size()), argv.data(),
                                          rcl_get_default_allocator(), &rcl_args);
  auto rcl_context =
  ...
  if (rcl_ret != RCL_RET_OK) {
    RCLCPP_ERROR_STREAM(this->dataPtr->node_->get_logger(),
                        "Argument parser error: " << rcl_get_error_string().str);
    rcl_reset_error();
    return;
  }
  ...
  RCLCPP_DEBUG_STREAM(this->dataPtr->node_->get_logger(),
                      "[microros_ros2_control] Setting up controller for ["
  ...
  if (enabledJoints.size() == 0) {
    RCLCPP_DEBUG_STREAM(this->dataPtr->node_->get_logger(),
                        "[microros_ros2_control] There are no available Joints.");
    return;
  }
  ...
  try {
    ...
    RCLCPP_ERROR_STREAM(this->dataPtr->node_->get_logger(),
                        "Error parsing URDF in microros_ros2_control plugin, plugin not active "
                        ": " << ex.what());
    return;
  }
  ...
  try {
    this->dataPtr->robot_hw_sim_loader_.reset(
        new pluginlib::ClassLoader<microros_ros2_control::GazeboSimSystemInterface>(
            "microros_ros2_control", "microros_ros2_control::GazeboSimSystemInterface"));
  ...
  for (unsigned int i = 0; i < control_hardware_info.size(); ++i) {
    ...
    std::unique_ptr<microros_ros2_control::GazeboSimSystemInterface> gzSimSystem;
    RCLCPP_DEBUG(this->dataPtr->node_->get_logger(), "Load hardware interface %s ...",
                 robot_hw_sim_type_str_.c_str());

    try {
      gzSimSystem = std::unique_ptr<microros_ros2_control::GazeboSimSystemInterface>(
    ...
    if (!gzSimSystem->initMicroRos(this->dataPtr->node_, enabledJoints, control_hardware_info[i],
                                   _ecm, this->dataPtr->update_rate)) {
...
//////////////////////////////////////////////////
void GazeboSimROS2ControlPlugin::PreUpdate(const sim::UpdateInfo& _info,
